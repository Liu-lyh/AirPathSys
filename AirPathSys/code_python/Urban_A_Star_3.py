...
class FlightVisualizer:
    ...
    def draw_waypoints(self, waypoints, goal_points=[]):
        self.client.simPlotPoints(
            waypoints,
            color_rgba=[0.0, 1.0, 0.0, 1.0],  # 绿色航路点
            size=15,
            is_persistent=True
        )
        self.client.simPlotPoints(
            goal_points,
            color_rgba=[1.0, 1.0, 0.0, 1.0],  # 黄色目标点
            size=30,
            is_persistent=True
        )
    ...
    def get_trajectory(self):
        return [(p.x_val, p.y_val, p.z_val) for p in self.trail_points]
...
def visualize_path(grid, path, start, goals, filename='path_on_grid_map.png'):
    ...
    # 1) 画路径
    ys, xs = zip(*path)
    plt.plot(xs, ys, '-', color='cyan', linewidth=2, label='路径')

    # 2) 标注起点
    sy, sx = start
    plt.scatter(sx, sy, c='green', s=100, marker='o', label='起点')

    # 3) 标注所有目标（倒数最后一个为最终目标，其余为中间目标）
    for i, (gy, gx) in enumerate(goals):
        if i < len(goals) - 1:
            plt.scatter(gx, gy, c='yellow', s=150, marker='*',
                        label='中间目标' if i == 0 else None)
        else:
            plt.scatter(gx, gy, c='red', s=150, marker='X', label='最终目标')
...
def move_by_path_tracking(client, waypoints, goal_points, Va, z_val=-5, epsilon=1.0, dt=0.1):
    ...
    return viz.get_trajectory()  # 无论正常还是中断，均返回已记录轨迹
...
def plot_trajectory(data, goal_points):
    ...
    ax.plot(x, y, z, color='red', label='飞行轨迹', linewidth=2)
    ...
    ax.set_zlim(-10,0)      # ✅ 反转Z轴范围
    ax.invert_zaxis()       # ✅ 显式反转Z轴方向
    ax.legend()
    plt.show()
...
def main():
    ...
    # 飞行：根据航路点追踪飞行
    print("🚁 飞行这一段 …")
    trajectory_data = move_by_path_tracking(client, flight_waypoints, all_goal_points, Va=3, z_val=-5)
    # 4. 最后降落
    print("✅ 目标到达，开始降落")
    ...
    # 绘制3D轨迹（确保在主程序尾部执行）
    print("📊 绘制3D轨迹 ...")
    plot_trajectory(trajectory_data, [(p.x_val, p.y_val, p.z_val) for p in all_goal_points])
