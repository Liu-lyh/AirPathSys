...
# ---------- RRT 树 + 最终路径可视化 ----------
# ---------- RRT 树 + 最终路径可视化 ----------
def visualize_rrt_and_path(grid, nodes, final_path, start, goal, viz, w, h):
    ...
    # 保存图像到当前目录
    output_path = './rrt_path_with_goal.png'
...
# ---------- 航路点追踪飞行 ----------
def move_by_path_tracking(client, waypoints, goal_points,Va, z_val=-5, epsilon=2.0, dt=0.1):
...
def plot_3d_flight_model(trajectory, goals):
    ...
    ax.set_title('3D Multi-Goal Flight Path'); ax.legend()
    plt.show()
...
def main():
    ...
    plt.title("Inflated Grid"); plt.show()
    ...
    pixs   = extract_waypoints_by_distance(interp, 20)
    flight_wps = [
        Vector3r((x - w//2)*0.3, -(y - h//2)*0.3, -5)
        for y,x in pixs
    ]
